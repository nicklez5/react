...
function AddWomenShirt(props) {
  const [name, setWomenShirtName] = useState('');
    ...
    className="block m-2 bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-11 rounded">
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Add WomenShirt</Modal.Title>
        </Modal.Header>
        <Modal.Body>
        <form onSubmit={(e) => {
            e.preventDefault()
            ...
            setPrice('')
            setQuantity('')
            props.newWomenShirt(name,size,img,color,price,quantity)
        }} 
        id="editmodal" className="w-full max-w-sm m-3">
            <div className="md:flex md:items-center mb-6">
                ...
                placeholder="T-shirt"
                value={name}
                onChange={(e) => {setWomenShirtName(e.target.value)}}
                />
                </div>
            </div>
            <div className="md:flex md:items-center mb-6">
