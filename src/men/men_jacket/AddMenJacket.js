...
function AddMenJacket(props) {
  const [name, setMenJacketName] = useState('');
    ...
    className="block m-2 bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-11 rounded">
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Add MenJacket</Modal.Title>
        </Modal.Header>
        <Modal.Body>
        <form onSubmit={(e) => {
            e.preventDefault()
            ...
            setPrice('')
            setQuantity('')
            props.newMenJacket(name,size,img,color,price,quantity)
        }} 
        id="editmodal" className="w-full max-w-sm m-3">
            <div className="md:flex md:items-center mb-6">
                ...
                value={name}
                onChange={(e) => {setMenJacketName(e.target.value)}}
                />
                </div>
            </div>
            <div className="md:flex md:items-center mb-6">
